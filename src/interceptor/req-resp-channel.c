...
open_chan()
{
    /* 1. Prepare the required sockaddr_un data structure */
    ...
    {
        printf("-- %s:%d: [INFO] new req-resp-chan_t {fd = %d}\n",
...
void
chan_send(chan_t ch, const Request* req)
{
...
void
chan_recv(chan_t ch, Response** response)
{
    static __thread char buf[RPC_MESSAGE_MAX_SIZE];
