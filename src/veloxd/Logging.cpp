    ...
    logging::value_ref<int, tag::lineno> const& line)
{
    ...
    // snprintf writes an extra NULL at the end of the string; the +1 byte here
    // is reserved specifically for snprintf's NULL terminator, otherwise it would overflow and segfault...
...
LoggingInitializer::run()
{
    ...
    if (logging2stdout) {
        ...
        (
             keywords::file_name = "veloxd.%N.log",
